...
public class clossnessCentrality {
	...
	public clossnessCentrality(String path) {
		numVertices = 0;
		numEdges = 0;
	...
	public void calculate_clossness() {
		for(Integer s: vertex_num.values()) {
			core(s);
		}
	}
	
	public void core(int s) {
		double result = 0;
			...
			for(int i=0; i < numVertices; i++) {
				if(adjMartrix[i][cur] == 1 || adjMartrix[cur][i] == 1) {
					if(!visited.contains(i)) {
						visited.add(i);
						myQue.add(i);
						distance.put(i, distance.get(cur) + 1);
					}					
				}
			}
		}
		for(Integer a: distance.values()) {
			if(a != 0) {
				result = result + 1.0/a;
			}
		}
		result = result/(numVertices-1);
	...
	public static void main(String args[]) {
		clossnessCentrality cc = new clossnessCentrality(args[0]);
		cc.readData();
//		cc.testData();
		cc.calculate_clossness();
		cc.printResult();
	} 
